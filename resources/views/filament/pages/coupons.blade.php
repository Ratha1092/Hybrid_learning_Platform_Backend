@php
    ...
    $tabColors = ['all' => '#6366f1', 'active' => '#16a34a', 'expired' => '#dc2626', 'disabled' => '#64748b', 'trashed' => '#94a3b8'];
    ...
    <div class="lp-card lpa lp2">
        ...
        <div class="lp-toolbar">
            ...
            <div class="lp-search-box">
                ...
                <input type="text" wire:model.live.debounce.500ms="search" placeholder="Search code...">
            </div>
        </div>

        {{-- Table --}}
        <div style="overflow-x:auto;border-radius:0 0 12px 12px;" wire:loading.class="lp-loading" wire:target="selectStatus,gotoPage,search,setPerPage">
        <table class="lp-table">
            ...
            <tbody>
                ...
                @endphp
                <tr wire:key="coupon-row-{{ $coupon->id }}" @unless($coupon->trashed()) class="lp-row-link" onclick="Livewire.navigate('{{ $viewUrl($coupon) }}')" @endunless>
                    ...
                    <td>
                        @if($coupon->trashed())
                        <span class="lp-badge" style="background:rgba(148,163,184,.12);color:#64748b">
                            <span class="lp-dot" style="background:#64748b"></span>
                            Deleted
                        </span>
                        @else
                        ...
                        <div class="lp-actions">
                            @if($coupon->trashed())
                                ...
                                <button type="button" class="lp-act-btn" title="Restore"
                                    wire:click="restoreCoupon({{ $coupon->id }})"
                                    wire:confirm="Restore the &quot;{{ addslashes($coupon->code) }}&quot; coupon?">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3"/>
                                    </svg>
                                </button>
                                <button type="button" class="lp-act-btn" title="Delete permanently" style="color:#dc2626"
                                    wire:click="forceDeleteCoupon({{ $coupon->id }})"
                                    wire:confirm="Permanently delete the &quot;{{ addslashes($coupon->code) }}&quot; coupon? This cannot be undone.">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                                ...
                                @endif
                            @else
                                <a href="{{ $viewUrl($coupon) }}" wire:navigate class="lp-act-btn" title="View">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                                ...
                                @if($canUpdate)
                                <a href="{{ $editUrl($coupon) }}" wire:navigate class="lp-act-btn" title="Edit">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                                ...
                                @endif
                                @if($canDelete)
                                <button type="button" class="lp-act-btn" title="Delete" style="color:#dc2626"
                                    wire:click="deleteCoupon({{ $coupon->id }})"
                                    wire:confirm="Delete the &quot;{{ addslashes($coupon->code) }}&quot; coupon? You can restore it later from the Deleted tab.">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/>
                                    </svg>
                                </button>
