...
<div class="rp">
    <div class="rp-header">
        <div>
            <h1>Instructor Intelligence Report</h1>
            <p>Revenue, students, and ratings per instructor.</p>
        </div>
        <div class="rp-actions">
            <div class="rp-drp-wrap" wire:key="rp-drp-instructor" x-data="rpDate({preset:'{{ $activePreset }}',from:'{{ $activeDateFrom }}',to:'{{ $activeDateTo }}',extras:{}})" data-rp-preset="{{ $activePreset }}" data-rp-from="{{ $activeDateFrom }}" data-rp-to="{{ $activeDateTo }}" @click.outside="open=false">
            ...
            <a href="{{ route('admin.reports.csv', array_filter(['type' => 'instructor_intelligence', 'preset' => $activePreset, 'date_from' => $activePreset === 'custom' ? $activeDateFrom : null, 'date_to' => $activePreset === 'custom' ? $activeDateTo : null])) }}" class="rp-btn">Export CSV</a>
            <a href="{{ route('admin.reports.pdf', array_filter(['type' => 'instructor_intelligence', 'preset' => $activePreset, 'date_from' => $activePreset === 'custom' ? $activeDateFrom : null, 'date_to' => $activePreset === 'custom' ? $activeDateTo : null])) }}" class="rp-btn">Export PDF</a>
    ...
    <div class="rp-kpi-grid">
        <div class="rp-kpi-card"><div class="rp-kpi-label">Total Instructors</div><div class="rp-kpi-value">{{ number_format($kpis['totalInstructors']) }}</div></div>
        <div class="rp-kpi-card"><div class="rp-kpi-label">Total Earnings</div><div class="rp-kpi-value">${{ number_format($kpis['totalEarnings'], 2) }}</div></div>
        <div class="rp-kpi-card"><div class="rp-kpi-label">Avg. Earnings</div><div class="rp-kpi-value">${{ number_format($kpis['averageEarnings'], 2) }}</div></div>
        <div class="rp-kpi-card"><div class="rp-kpi-label">Avg. Rating</div><div class="rp-kpi-value">{{ $kpis['averageRating'] }}★</div></div>
        <div class="rp-kpi-card"><div class="rp-kpi-label">Top Earner</div><div class="rp-kpi-value" style="font-size:14px;">{{ \Illuminate\Support\Str::limit($kpis['topEarner'], 18) }}</div></div>
    </div>

    <div class="rp-table-card">
        <table class="rp-table">
            <thead>
                <tr><th>Instructor</th><th>Courses</th><th>Students</th><th>Earnings</th><th>Wallet Balance</th><th>Rating</th></tr>
            </thead>
            <tbody>
                @forelse($instructors as $instructor)
                    <tr>
                        ...
                        <td>{{ number_format($instructor['studentCount']) }}</td>
                        <td>${{ number_format($instructor['earnings'], 2) }}</td>
                        <td>${{ number_format($instructor['walletBalance'], 2) }}</td>
                        <td>{{ $instructor['avgRating'] }}★</td>
                    </tr>
                @empty
                    <tr><td colspan="6" style="text-align:center;color:var(--t2);padding:24px;">No instructor data found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
@if(\App\Support\PanelAccess::can('reports.schedule'))
    @include('filament.pages.partials._schedule-modal', ['reportLabel' => 'Instructor Intelligence'])
@endif
