...
.re-btn-primary{background:#6366f1;color:#fff}
...
.re-hero-icon{width:56px;height:56px;border-radius:14px;display:grid;place-items:center;flex-shrink:0;background:rgba(99,102,241,.15);color:#6366f1}
...
.re-input.re-input-error{border-color:#ef4444}
...
.re-search-wrap{position:relative;margin:16px 0 4px}
.re-search-icon{position:absolute;left:11px;top:50%;transform:translateY(-50%);width:14px;height:14px;color:var(--t2);pointer-events:none}
.re-search{width:100%;background:var(--p2);border:1px solid var(--bd2);border-radius:8px;padding:8px 12px 8px 32px;font-size:13px;color:var(--t1);font-family:inherit;outline:none;transition:border-color .15s}
.re-search:focus{border-color:#6366f1}
.re-search::placeholder{color:var(--t2)}

.re-modules{display:grid;gap:14px;margin-top:6px}
.re-module{border:1px solid var(--bd);border-radius:10px;overflow:hidden}
.re-module[data-hidden="true"]{display:none}
...
.re-perm-label:has(input:checked){background:rgba(99,102,241,.12);border-color:rgba(99,102,241,.3);color:#6366f1}
.re-perm-label input{width:14px;height:14px;accent-color:#6366f1;cursor:pointer}
...
.re-no-results{padding:24px;text-align:center;font-size:12.5px;color:var(--t2);display:none}
