@php
    ...
    $users = $role->users()->orderBy('name')->limit(12)->get();
...
.rv-user-row{display:flex;align-items:center;gap:10px;padding:8px;border-radius:8px;text-decoration:none;color:inherit;transition:background .12s}
    ...
    <div class="rv-card rva rv3">
        ...
        <div class="rv-users">
            @forelse ($users as $u)
                ...
                @endphp
                <a href="{{ route('filament.admin.resources.users.view', ['record' => $u->id]) }}" class="rv-user-row">
                    <img src="{{ $avUrl }}" class="rv-user-avatar" alt="">
                    <div>
                        <div class="rv-user-name">{{ $u->name }}</div>
                        <div class="rv-user-email">{{ $u->email }}</div>
                    </div>
                </a>
            @empty
            ...
            @if($usersCount > $users->count())
                <div class="rv-users-more">+{{ number_format($usersCount - $users->count()) }} more</div>
            @endif
        </div>
    </div>
</div>
